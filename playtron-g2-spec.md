...
**ノート番号とチャンネルのマッピングは接続後に実機確認すること。**  
MIDIモニターツール（[Morning Star](https://morningstar.io/midimon) 等）で確認できる。
...
#### オブジェクト（チャンネル）による音色の違い

MIDIチャンネル番号（どの端子に繋いだオブジェクトか）で音域と音色を変える。サンプラーではなく**音のキャラクター**を変える。
...
| 項目 | 状況 |
|---|---|
| Playtronのデバイス名（MIDIデバイス名） | 接続後に `easymidi.getInputs()` で確認 |
| G2でのテキスト更新速度の上限 | 実機検証が必要 |
| ラッチ機能のフィードバック表示 | G2上でのON/OFF表示方法を要検討 |
| チャンネルとオブジェクトの最終的な割り当て | セットアップ時に調整 |
